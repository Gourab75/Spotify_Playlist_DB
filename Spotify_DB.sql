...
SELECT Name,Email FROM users LEFT JOIN playlists ON users.UserId = playlists.UserId;

-- right join
-- all records from the right table(users) AND  the matching records from the left table (playlists).
SELECT Name,Email FROM users right JOIN playlists ON users.UserId = playlists.UserId;

-- inner join
-- selects records that have matching values in both tables 
SELECT Name,Email FROM users INNER JOIN playlists ON users.UserId = playlists.UserId;
